...
export function Documents() {
  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-3xl text-charcoal-900 sm:text-4xl">My documents</h1>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-5">
        ...
        {error && (
          <p className="mt-3 rounded-xl bg-destructive-50 px-3.5 py-2.5 text-sm text-destructive-600">{error}</p>
        )}
      </Card>
    ...
    <li className="flex items-start gap-3.5 p-4">
      <span
        className={cx(
          ...
          isImage ? 'bg-sage-50 text-sage-600' : 'bg-primary-50 text-primary-600',
        )}
      >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-medium text-charcoal-900">{kind?.label ?? 'Document'}</p>
        <p className="truncate text-sm text-charcoal-500">{doc.fileName}</p>
        ...
        {usedFor.length > 0 && (
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            <span className="text-xs text-charcoal-400">Used for:</span>
            {usedFor.map((task) => (
              <Link key={task.id} to={`/app/task/${task.id}`}>
                <Badge className="hover:border-primary-300">{task.title}</Badge>
              </Link>
            ))}
          </div>
        )}
  ...
  return (
    <section>
        ...
        {needed.map((kind) => {
          ...
          return (
            <div key={kind.id} className="flex items-start gap-3 p-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-medium text-charcoal-900">{kind.label}</p>
                <p className="mt-0.5 text-sm text-charcoal-500">{kind.guidance}</p>
      ...
      <Callout tone="neutral" icon={<Info size={16} />} className="mt-4">
        ...
        secure. AfterIDo never asks for the numbers on them.
      </Callout>
    </section>
  );
}
