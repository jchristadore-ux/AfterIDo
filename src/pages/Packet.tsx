...
export function Packet() {
  return (
    <div className="space-y-6">
      <Link
        to="/app"
        className="no-print inline-flex items-center gap-1.5 text-sm text-ink-500 hover:text-ink-900"
      >
        <ArrowLeft size={15} /> Dashboard
      </Link>

      <header className="no-print">
        <h1 className="text-3xl text-ink-900 sm:text-4xl">Your name-change packet</h1>
  ...
  return (
    <div className="space-y-6">
      <Card className="no-print p-5">
        <p className="mb-3 text-sm font-medium text-ink-700">What to include</p>
      ...
      {/* ---------------------------------------------------- The document */}
      <article className="print-page rounded-card border border-ink-100 bg-paper-raised p-7 shadow-soft sm:p-10">
        {/* Cover */}
        <header className="print-avoid-break border-b border-ink-200 pb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-rose-600">
            NameDay · Name change packet
          </p>
        ...
        <PacketSection n="1" title="Your information">
          ...
          <p className="mt-5 text-xs text-ink-400">
            ...
            NameDay never collects them. Have those on hand separately when you file.
          </p>
        </PacketSection>
          ...
          <ul className="space-y-2.5">
            {neededDocs.map((kind) => (
              <li key={kind.id} className="flex items-start gap-3">
                <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border border-ink-400 text-[10px] leading-none">
                  {heldKinds.has(kind.id) ? '✓' : ''}
                </span>
                <span>
                  <span className="font-medium text-ink-900">{kind.label}</span>
                  <span className="block text-sm text-ink-500">{kind.guidance}</span>
                </span>
              </li>
            ))}
          </ul>
          ...
          <div className="space-y-8">
            {phases.map((phase) => (
              <div key={phase.n}>
                ...
                <p className="mb-4 text-sm text-ink-500">{phase.caption}</p>

                <div className="space-y-5">
                  {phase.tasks.map((task) => (
                    <div
                      key={task.id}
                      className="print-avoid-break rounded-xl border border-ink-100 p-4"
                    >
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <p className="font-medium text-ink-900">
                          <span className="mr-2 inline-block h-3.5 w-3.5 shrink-0 rounded-sm border border-ink-400 align-middle text-[9px] leading-3">
                      ...
                      <p className="mt-1.5 text-sm text-ink-600">{task.summary}</p>

                      {task.whatYouNeed.length > 0 && (
                        <p className="mt-2.5 text-sm text-ink-600">
                          <span className="font-medium text-ink-900">Bring: </span>
                          {task.whatYouNeed.join('; ')}
                        </p>
                      )}

                      <ol className="mt-2.5 list-decimal space-y-1 pl-5 text-sm text-ink-600">
                        {task.steps.map((s) => (
                          <li key={s}>{s}</li>
                        ))}
                      </ol>
                      ...
                      {task.officialLinks.length > 0 && (
                        <p className="mt-1.5 break-words text-xs text-ink-500">
                      ...
                      {task.state.instances.length > 0 && (
                        <p className="mt-2 text-sm text-ink-600">
                          <span className="font-medium text-ink-900">Accounts: </span>
                      ...
                      {includeNotes && task.state.notes.trim() && (
                        <p className="mt-2 rounded-lg bg-paper-sunk px-3 py-2 text-sm text-ink-700">
        ...
        <footer className="mt-10 border-t border-ink-200 pt-5">
          <p className="text-xs leading-relaxed text-ink-500">{DISCLAIMER_TEXT}</p>
          <p className="mt-2 text-xs text-ink-400">
            Generated by NameDay from information you provided. Verify every requirement on the
...
function PacketSection({
  ...
  return (
    <section className="print-break mt-9">
      <h3 className="mb-4 flex items-baseline gap-3 font-display text-xl text-ink-900">
        <span className="text-sm text-rose-600">{n}</span>
        {title}
      </h3>
      {children}
    </section>
...
function Row({
  ...
  return (
    <div>
      ...
      <dd
        className={
          multiline ? 'mt-0.5 whitespace-pre-line text-ink-900' : 'mt-0.5 text-ink-900'
        }
      >
        {value.trim() || '—'}
      </dd>
